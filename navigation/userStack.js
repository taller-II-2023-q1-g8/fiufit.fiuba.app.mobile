...
export default function UserStack({ email, token }) {
  ...
  const fetchUser = async () => {
    const response = await fetch('https://api-gateway-k1nl.onrender.com/user?email=' + email, {
      method: 'GET',
      headers: {
  ...
  return loading ? (
    ...
    <StateProvider initialState={data} reducer={reducer}>
      <NavigationContainer>
        <Stack.Navigator>
          <Stack.Screen component={HomeScreen} name={texts.Home.name} options={defaultNavigationOptions} />
          <Stack.Screen
